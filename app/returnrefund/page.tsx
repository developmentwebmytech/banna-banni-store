...
export default function ReturnRefundPolicyPage() {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const res = await fetch("/api/admin/returnrefundpolicy", {
          cache: "no-store",
        })
  ...
  return (
    <div className="flex flex-col w-full bg-gray-100 min-h-screen">
      ...
      <div className="bg-teal-800 text-white text-center py-14">
        <h1 className="text-3xl font-semibold">{data?.title || "Return & Refund Policy"}</h1>
      </div>
      ...
      <div className="w-full px-4 py-10">
        <div className="bg-white rounded-lg shadow-lg w-full p-8 text-[17px] leading-relaxed space-y-6">
          {loading ? (
            ...
            <div className="text-center text-red-500">No Return & Refund Policy found.</div>
          )}
        </div>
      </div>
    </div>
