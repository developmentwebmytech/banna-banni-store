...
const productOptions = ["3pc Lehenga", "Blouses", "1pc Kurti", "2pc Kurti", "3pc Kurti", "Petticoat"]
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setErrors([{ field: "submit", message: errorData.error || "Failed to create wholesaler" }])
        return
      }

      router.push("/shop-manager-dashboard/wholesalers")
    } catch (error) {
      setErrors([{ field: "submit", message: "An error occurred while creating the wholesaler" }])
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Enter the details for the new wholesaler.</CardDescription>
      </CardHeader>
