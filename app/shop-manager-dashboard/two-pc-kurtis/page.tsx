...
export default function TwoPcKurtiList() {
  const [kurtis, setKurtis] = useState<Kurti[]>([])

  const fetchKurtis = () => {
    fetch("/api/admin/two-pc-kurtis")
      .then((res) => res.json())
      .then(setKurtis)
  }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this 2pc Kurti entry?")) {
      await fetch(`/api/admin/two-pc-kurtis/${id}`, {
        method: "DELETE",
      })
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-2xl font-bold">2pc Kurti Entries</CardTitle>
        <Link href="/shop-manager-dashboard/two-pc-kurtis/new">
          <Button>
              ...
              <TableRow key={k._id}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Link href={`/shop-manager-dashboard/two-pc-kurtis/${k._id}`}>
