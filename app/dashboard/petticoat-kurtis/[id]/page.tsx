...
export default function EditPetticoatKurti() {
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/admin/petticoat-kurtis/${id}`)
        .then((res) => res.json())
        .then((data) => {
          setFinancialYear(data.financialYear || "")
          setWholesalerId(data.wholesalerId?._id || "")
          setPetticoatFabricType(data.petticoatFabricType || "")
          setWork(data.work || "")
          setWaistSize(data.waistSize || "")
          setPetticoatLength(data.petticoatLength || "")
  ...
  const handleUpdate = async () => {
    ...
    router.push("/dashboard/petticoat-kurtis")
  }

  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Edit Petticoat Kurti Entry</CardTitle>
        <CardDescription>Update the details for this Petticoat Kurti entry.</CardDescription>
      </CardHeader>
