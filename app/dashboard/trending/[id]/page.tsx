...
// Define interfaces for trending product data
...
export default function TrendingEditPage({ params }: PageProps) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Convert the response to our TrendingProductData interface
      setProductData({
        _id: data._id,
        images: data.images || [],
        title: data.title || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validation
      const requiredFields: (keyof TrendingProductData)[] = [
        "title",
        "description",
        "category",
        "price",
      ...
      const apiUrl = isNew ? "/api/admin/trending" : `/api/admin/trending/${id}`
    ...
    <div className="space-y-6 px-10 py-16">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <h1 className="text-2xl font-semibold">{isNew ? "Create New Trending Product" : "Edit Trending Product"}</h1>
        </div>
