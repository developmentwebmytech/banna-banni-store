...
export default function NewArrivalEditPage({ params }: PageProps) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Convert the response to our ProductData interface
      setProductData({
        _id: data._id,
        images: data.images || [],
        title: data.title || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const apiUrl = isNew ? "/api/admin/newarrivals" : `/api/admin/newarrivals/${id}`
      ...
      router.push("/dashboard/newarrivals")
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => router.push("/dashboard/newarrivals")}
            disabled={isSaving}
          >
            Cancel
          </Button>
