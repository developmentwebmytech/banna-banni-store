...
export default function EditTwoPcKurti() {
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/admin/two-pc-kurtis/${id}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleUpdate = async () => {
    ...
    router.push("/dashboard/two-pc-kurtis")
  }

  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Edit 2pc Kurti Entry</CardTitle>
        <CardDescription>Update the details for this 2pc Kurti entry.</CardDescription>
      </CardHeader>
