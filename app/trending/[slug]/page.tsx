...
interface ITrendingVariation {
  color: string
  size: string
  stock: number
  sku?: string
  price_modifier?: number // Add price_modifier to variations
}

// Create a plain data type without Mongoose methods
type TrendingProductData = {
  _id: string
  title: string
  images: string[]
  description: string
...
const sampleProduct: TrendingProductData = {
  _id: "sample-trending-product",
  title: "Trending Designer Ethnic Collection",
  images: [
  ...
  description:
    "Stunning trending designer ethnic wear that combines traditional elegance with modern style. Perfect for festivals, weddings, and special occasions. Made with premium quality fabric and intricate detailing.",
  category: "Ethnic Wear",
  price: 4500,
  mrp: 8999,
  discount: "50% OFF",
  ratings: 4.3,
  slug: "trending-designer-ethnic-collection",
  variations: [
    { color: "Red", size: "S", stock: 3, price_modifier: 0 },
    { color: "Red", size: "M", stock: 8, price_modifier: 0 },
    { color: "Red", size: "L", stock: 5, price_modifier: 0 },
    { color: "Blue", size: "S", stock: 6, price_modifier: 50 },
    { color: "Blue", size: "M", stock: 12, price_modifier: 50 },
    { color: "Blue", size: "L", stock: 4, price_modifier: 50 },
    { color: "Green", size: "S", stock: 0, price_modifier: 0 }, // Out of stock example
    { color: "Green", size: "M", stock: 2, price_modifier: 0 },
    { color: "Green", size: "L", stock: 7, price_modifier: 0 },
    { color: "Pink", size: "S", stock: 9, price_modifier: 0 },
    { color: "Pink", size: "M", stock: 15, price_modifier: 0 },
    { color: "Pink", size: "L", stock: 6, price_modifier: 0 },
  ],
  createdAt: new Date(),
  updatedAt: new Date(),
}
...
function TrendingProductDetailClient({ slug }: { slug: string }) {
  const { addToCart } = useCart()
  const { toast } = useToast()

  const [product, setProduct] = useState<TrendingProductData | null>(null)
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        console.log(`Fetching trending product with slug: ${slug}`)
        const response = await fetch(`/api/trending/${slug}`)

        if (!response.ok) {
          console.warn("Trending API failed, using sample product")
        ...
        console.warn("Trending API error, using sample product:", err.message)
  ...
  useEffect(() => {
    ...
    if (product.variations && product.variations.length > 0) {
      ...
      const inStockVariations = product.variations.filter((v: ITrendingVariation) => v.stock > 0)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      {/* Modal Lightbox - same as existing implementation */}
      {isModalOpen && (
...
export default function TrendingProductDetailPage({
  params,
}: {
  params: { slug: string }
}) {
  return <TrendingProductDetailClient slug={params.slug} />
}
